api.kraken <- function(command, args = list()) {
  ...
  ret <- POST("https://kraken.com/tradingApi",
...
api.kraken.public <- function(command){
  req <- paste0("https://kraken.com/public?command=",command)
  ret <- POST(req)
  stop_for_status(ret)
  return(content(ret))
}
...
returnOpenOrders <- function(currency.pair="all"){
  ...
  open.orders <- api.kraken(command=command, args=config.specs)
  return(open.orders)
}
...
returnBalances <- function(){
  ...
  balances <- api.kraken(command=command)
  return(balances)
}
...
returnCompleteBalances <- function(account="exchange"){
  ...
  balances <- api.kraken(command=command, args=config.specs)
  return(balances)
}
...
returnTradeHistory <- function(currency.pair="all"){
  ...
  config.specs <- list(currencyPair=currency.pair)
  trade.history <- api.kraken(command=command, args=config.specs)
...
cancelOrder <- function(order.number=NULL){
  ...
  command.result <- api.kraken(command=command, args=config.specs)
...
returnMarginAccountSummary <- function(){
  ...
  command.result <- api.kraken(command=command)
  return(command.result)
}
...
returnTicker <- function(){
  ...
  # 
  # Call: https://kraken.com/public?command=returnTicker
  command.result <- api.kraken.public(command)
  return(command.result)
}